...
import { Loader2, Send, Paperclip, Code, Download } from "lucide-react"
...
export default function CreateWebsitePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // הוספת לוג לקונסול
    setConsoleLogs((prev) => [...prev, `בקשה: ${input}`])
    ...
    try {
      ...
      const assistantMessage = {
        role: "assistant",
        content: `אני אעזור לך ליצור אתר בהתאם לתיאור שלך. אני מתחיל לעבוד על קוד ה-HTML, CSS ו-JavaScript הדרושים.`,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      }
      ...
      // הוספת לוג לקונסול
      setConsoleLogs((prev) => [...prev, "מערכת: יוצר קוד HTML..."])
      ...
      const codeMessage = {
        role: "assistant",
        content: "הנה הקוד שיצרתי עבור האתר שלך. אתה יכול להוריד אותו או לצפות בתצוגה מקדימה.",
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      }
      ...
      setConsoleLogs((prev) => [...prev, `שגיאה: אירעה שגיאה ביצירת האתר - ${error.message || error}`])

      // הוספת הודעת שגיאה
      const errorMessage = {
        role: "assistant",
        content: `אירעה שגיאה ביצירת האתר: ${error.message || "אנא נסה שוב"}`,
        timestamp: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }),
      }

      setMessages((prev) => [...prev, errorMessage])
      setError(error.message || "אירעה שגיאה ביצירת האתר")
    } finally {
    ...
    <div className="flex flex-col items-center justify-between p-4 md:p-8">
      <Card className="w-full max-w-4xl">
        <CardHeader className="bg-gradient-to-r from-purple-500 to-blue-500 text-white rounded-t-lg">
          <CardTitle className="text-xl md:text-2xl font-bold">יצירת אתר</CardTitle>
        </CardHeader>
            ...
            <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4 h-[50vh] overflow-y-auto">
              {messages.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-full text-center text-gray-500 dark:text-gray-400">
                  ...
                  <h3 className="text-lg font-medium mb-2">ברוכים הבאים ליוצר האתרים</h3>
                  <p className="max-w-md">תאר את האתר שברצונך ליצור, או העלה תמונות וקבצים לעזרה בתהליך היצירה.</p>
                </div>
              ) : (
                <div className="space-y-4">
            ...
            <form onSubmit={handleSubmit} className="flex gap-2">
              ...
              <div className="flex-1">
                <Textarea
                  ...
                  placeholder="תאר את האתר שברצונך ליצור..."
