...
export async function POST(request: Request) {
  try {
    const { prompt, appType, apiKey, attachment } = await request.json()
    ...
    const response = await anthropic.messages.create({
      ...
      messages: [
        {
          role: "user",
          content: `צור קוד React לאפליקציית ${appType || "web"} בהתאם לתיאור הבא: ${fullPrompt}. 
        הקוד צריך להיות מלא ומוכן להרצה. השתמש בעברית בממשק המשתמש.`,
        },
      ],
      system: `אתה מומחה לפיתוח אפליקציות. תפקידך ליצור קוד React מלא ומוכן להרצה בהתאם לתיאור שהמשתמש מספק.
    הקוד צריך להיות מותאם לעברית ולכלול את כל הסגנונות והפונקציונליות הנדרשים.
    ...
    return NextResponse.json(
      {
        error: error.message || "Error generating app",
